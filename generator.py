...
class G3dGenerator(object):
    ...
    def prepare_mesh(self, obj: bpy.types.Object, use_armature: bool) -> GMeshGeneratorOptions:
        """
        Analyzes attributes and triangulates the mesh.

        Returns final evaluated mesh with attribute flags and vertex generation settings
        """
        ...
        if (self.use_shapekeys and obj.data.shape_keys):
            ...
            opt.shape = GShape(obj.data.name, obj.data.shape_keys)

            print(f'{obj.name} has shapekeys: {opt.shape.id}')
